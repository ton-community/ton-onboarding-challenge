...
import {buildNftGiverDataCell, MineMessageParams, MiningData, NftGiverData, Queries, RoyaltyParams} from "./NftGiver.data";
import {Address, Cell, CellMessage, CommonMessageInfo, contractAddress, ExternalMessage, InternalMessage, Slice, toNano} from "ton";
import BN from "bn.js";
import {decodeOffChainContent} from "../lib/utils";
import { getNftGiverCodeCell } from "./NftGiver.code";

export class NftGiverLocal {
    private constructor(
    ...
    async getMiningData(): Promise<MiningData> {
        let res = await this.contract.invokeGetMethod('get_mining_data', [])

        if (res.type !== 'success') {
            throw new Error('Unable to invoke get_nft_content on collection')
        }

        let [powComplexity, lastSuccess, seed, targetDelta, minComplexity, maxComplexity] = res.result as [BN, BN, BN, BN, BN, BN]

        return {
            powComplexity,
            lastSuccess: lastSuccess.toNumber(),
            seed,
            targetDelta: targetDelta.toNumber(),
            minComplexity: minComplexity.toNumber(),
            maxComplexity: maxComplexity.toNumber(),
        }
    }
    ...
    async sendMineNft(from: Address, params: { queryId?: number } & MineMessageParams) {
        return await this.contract.sendInternalMessage(new InternalMessage({
            to: this.address,
            from,
            value: toNano(1),
            bounce: false,
            body: new CommonMessageInfo({
                body: new CellMessage(Queries.mine(params))
            }),
        }))
    }

    async sendRescaleComplexity(from: Address, params: { queryId?: number, expire: number }) {
        return await this.contract.sendInternalMessage(new InternalMessage({
            to: this.address,
            from,
            value: toNano(1),
            bounce: false,
            body: new CommonMessageInfo({
                body: new CellMessage(Queries.rescaleComplexity(params))
            })
        }))
    }

    static async createFromConfig(config: NftGiverData) {
        let code = await getNftGiverCodeCell()
        let data = buildNftGiverDataCell(config)
        ...
        return new NftGiverLocal(contract, address)
    }
    ...
    static async create(config: { code: Cell, data: Cell, address: Address }) {
        ...
        return new NftGiverLocal(contract, config.address)
    }
